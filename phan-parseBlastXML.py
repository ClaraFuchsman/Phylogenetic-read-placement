...
parser.add_option("-e", "--extract_hits", default = False, dest="extract_hits",
				  help="Specify db to extract hits from. Tested with blast db created by Blast+ "
				  		"version 2.2.8")
...
if extract_hits:
	...
	i = 0
	while i < num_cores:
		extract_hits_tmp_file_handles.append(open('{}_{}.parsetmp'.format(extract_hits_file, i), 'w+'))
		i += 1
...
for line in results_list:
	...
	if extract_hits:
		...
		if line[1] not in seen_results and line[1] != 'None':
			if line[1] == 'None':
				print(line)
			f = next(extract_hits_tmp_file_handle_cycle)
			f.write('{}\n'.format(line[1]))
...
if extract_hits:
	...
	for f in extract_hits_tmp_file_handles:
		...
		cmd ='blastdbcmd -db {} -entry_batch {} -out {}'.format(
...
cwd = os.getcwd()
for path, title, files in os.walk(cwd):
        for name in files:
                if '.parsetmp' in name:
